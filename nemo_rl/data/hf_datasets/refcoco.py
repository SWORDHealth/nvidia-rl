...
def prepare_refcoco_dataset(split: str = "default", seed: int = 42, task_name: Optional[str] = None, path_to_coco_images: Optional[Union[str, Path]] = None):
    ...
    if not os.path.exists(str(path_to_coco_images / "train2014")):
        ...
        download_and_unzip("http://images.cocodataset.org/zips/train2014.zip", str(path_to_coco_images))
    if not os.path.exists(str(path_to_coco_images / "val2014")):
        print(f"Downloading val2014 images to {path_to_coco_images}")
        download_and_unzip("http://images.cocodataset.org/zips/val2014.zip", str(path_to_coco_images))

    # add image column
    tr_dataset = tr_dataset.map(
        lambda example: {
            **example,
            "image_path": str(example["image_path"]).replace("coco/", str(path_to_coco_images) + "/") if "image_path" in example else example.get("image_path")
        }
    )
...
class RefCOCODataset:
    def __init__(self, split: str = "default", seed: int = 42, prompt_file: Optional[str] = None, task_name: str = "refcoco", path_to_coco_images: Optional[str] = None):
        ...
        if split not in VALID_SPLITS:
            raise ValueError(f"Invalid split: {split}. Please use one of {VALID_SPLITS}.")
        
        self.formatted_ds = prepare_refcoco_dataset(split=split, seed=seed, task_name=task_name, path_to_coco_images=path_to_coco_images)
