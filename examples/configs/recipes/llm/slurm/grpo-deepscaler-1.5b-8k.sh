#!/bin/bash
# SLURM batch script for NeMo RL Multi-Node GRPO Training - DeepScaler 1.5B 8K
# 4 nodes with 8 GPUs each

#SBATCH --job-name=nemo-rl-grpo-deepscaler-1.5b-8k
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-grpo-deepscaler-1.5b-8k-%j.out
#SBATCH --error=slurm_logs/nemo-rl-grpo-deepscaler-1.5b-8k-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_deepscaler_1.5b_8k.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_deepscaler_1.5b_8k.sh

echo "=== Starting Multi-Node GRPO SLURM Job - DeepScaler 1.5B 8K ==="
echo "Job ID: $SLURM_JOB_ID"
echo "Nodes: $SLURM_JOB_NUM_NODES"
echo "Node list: $SLURM_JOB_NODELIST"

# Launch the worker script on all nodes simultaneously using srun
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_deepscaler_1.5b_8k.sh
