...
#SBATCH --job-name=nemo-rl-sft-mind-qwen30ba3b-thinking-therapist-expert
#SBATCH --nodes=2
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-sft-mind-qwen30ba3b-thinking-therapist-expert-%j.out
#SBATCH --error=slurm_logs/nemo-rl-sft-mind-qwen30ba3b-thinking-therapist-expert-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_qwen30ba3b_thinking_mind_expert.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_qwen30ba3b_mind_node_${NODE_RANK}_$(date +%H%M%S).log"

    echo "📊 Starting training on node $NODE_RANK at $(date)" | tee -a "$LOG_FILE"

    # Run with detailed logging and real-time output
    uv run python examples/run_sft.py \
        --config examples/configs/recipes/llm/sft-mind-megatron-qwen30ba3b-thinking-therapist-expert.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_qwen30ba3b_thinking_mind_expert.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_qwen30ba3b_thinking_mind_expert.sh
