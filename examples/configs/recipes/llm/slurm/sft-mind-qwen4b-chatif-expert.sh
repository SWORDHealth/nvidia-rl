...
#SBATCH --job-name=nemo-rl-sft-mind-qwen4b-merged-chatif-expert
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-sft-mind-qwen4b-merged-chatif-expert-%j.out
#SBATCH --error=slurm_logs/nemo-rl-sft-mind-qwen4b-merged-chatif-expert-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_merged_chatif_expert.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    uv run python examples/run_sft.py \
        --config examples/configs/recipes/llm/sft-mind-megatron-qwen4b-merged-chatif-expert.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_merged_chatif_expert.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_merged_chatif_expert.sh
