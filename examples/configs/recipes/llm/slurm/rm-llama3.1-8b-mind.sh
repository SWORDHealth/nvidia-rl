#!/bin/bash
# SLURM batch script for NeMo RL Multi-Node Reward Model Training
# 4 nodes with 8 GPUs each

#SBATCH --job-name=nemo-rl-rm-llama3.1-8b-mind
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-rm-llama3.1-8b-mind-%j.out
#SBATCH --error=slurm_logs/nemo-rl-rm-llama3.1-8b-mind-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_rm_llama3.1_8b_mind.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    echo "=== Starting NeMo RL Reward Model Training ==="

    # Create a detailed log with timestamps and node info
    LOG_DIR="/home/pmartins/nemo-rl/slurm_logs/$(date +%Y%m%d)"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_rm_llama3.1_8b_mind_node_${NODE_RANK}_$(date +%H%M%S).log"

    echo "📊 Starting reward model training on node $NODE_RANK at $(date)" | tee -a "$LOG_FILE"

    # Run with detailed logging and real-time output
    uv run python examples/run_rm.py \
        --config examples/configs/recipes/llm/rm-llama3.1-8b-mind.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_rm_llama3.1_8b_mind.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_rm_llama3.1_8b_mind.sh
