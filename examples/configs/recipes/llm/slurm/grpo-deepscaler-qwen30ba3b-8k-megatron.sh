#!/bin/bash
# SLURM batch script for NeMo RL Multi-Node GRPO Training - DeepScaler Qwen30B 8K Megatron
# 4 nodes with 8 GPUs each

#SBATCH --job-name=nemo-rl-grpo-deepscaler-qwen30ba3b-8k-megatron
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-grpo-deepscaler-qwen30ba3b-8k-megatron-%j.out
#SBATCH --error=slurm_logs/nemo-rl-grpo-deepscaler-qwen30ba3b-8k-megatron-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_deepscaler_qwen30ba3b_8k_megatron.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    echo "=== Starting NeMo RL GRPO Training - DeepScaler Qwen30B 8K Megatron ==="

    # Create a detailed log with timestamps and node info
    LOG_DIR="/home/pmartins/nemo-rl/slurm_logs/$(date +%Y%m%d)"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/grpo_deepscaler_qwen30ba3b_8k_megatron_node_${NODE_RANK}_$(date +%H%M%S).log"

    echo "📊 Starting GRPO training - DeepScaler Qwen30B 8K Megatron on node $NODE_RANK at $(date)" | tee -a "$LOG_FILE"

    # Run with detailed logging and real-time output
    uv run python examples/run_grpo_math.py \
        --config examples/configs/recipes/llm/grpo-deepscaler-qwen30ba3b-8K_megatron.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_deepscaler_qwen30ba3b_8k_megatron.sh

echo "=== Starting Multi-Node GRPO SLURM Job - DeepScaler Qwen30B 8K Megatron ==="
echo "Job ID: $SLURM_JOB_ID"
echo "Nodes: $SLURM_JOB_NUM_NODES"
echo "Node list: $SLURM_JOB_NODELIST"

# Launch the worker script on all nodes simultaneously using srun
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_deepscaler_qwen30ba3b_8k_megatron.sh
