...
if [ "$NODE_RANK" -eq 0 ]; then
    echo "=== Starting NeMo RL GRPO Training with LLM Judge HTTP ==="

    # Create a detailed log with timestamps and node info
    LOG_DIR="/home/pmartins/nemo-rl/slurm_logs/$(date +%Y%m%d)"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/grpo_qwen4b_mind_llm_judge_http_node_${NODE_RANK}_$(date +%H%M%S).log"

    echo "🤖 Starting GRPO training with LLM Judge HTTP on node $NODE_RANK at $(date)" | tee -a "$LOG_FILE"

    # Run with detailed logging and real-time output
    uv run python examples/run_grpo_llm_judge_http.py \
        --config examples/configs/recipes/llm/grpo-qwen4b-mind-llm-judge-http.yaml \
        2>&1 | tee -a "$LOG_FILE"
