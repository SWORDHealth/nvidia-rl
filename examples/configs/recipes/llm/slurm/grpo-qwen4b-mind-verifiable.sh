...
#SBATCH --job-name=nemo-rl-grpo-qwen4b-mind-verifiable
#SBATCH --nodes=2
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-grpo-qwen4b-mind-verifiable-%j.out
#SBATCH --error=slurm_logs/nemo-rl-grpo-qwen4b-mind-verifiable-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_mind_verifiable.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    # Wait for worker nodes to connect (more time for 4 workers)
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/grpo_mind_verifiable_node_${NODE_RANK}_$(date +%H%M%S).log"

    echo "📊 Starting GRPO training with mind verifiable environment on node $NODE_RANK at $(date)" | tee -a "$LOG_FILE"
    echo "🧠 Using MindVerifiableEnvironment with custom reward functions:" | tee -a "$LOG_FILE"
    echo "   - Length reward: Encourages 50-200 word responses" | tee -a "$LOG_FILE"
    echo "   - Reflection reward: Rewards empathetic reflection patterns" | tee -a "$LOG_FILE"
    echo "   - Acknowledgment penalty: Penalizes formulaic acknowledgments" | tee -a "$LOG_FILE"
    echo "   - Content repetition penalty: Penalizes excessive word repetition" | tee -a "$LOG_FILE"

    # Run with detailed logging and real-time output
    uv run python examples/run_grpo_mind_verifiable.py \
        --config examples/configs/recipes/llm/grpo-qwen4b-mind-verifiable.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_mind_verifiable.sh

echo "=== Starting Multi-Node SLURM Job for GRPO Mind Verifiable Training ==="
echo "Job ID: $SLURM_JOB_ID"
echo "Nodes: $SLURM_JOB_NUM_NODES"
echo "Node list: $SLURM_JOB_NODELIST"
echo "🧠 Training with MindVerifiableEnvironment and custom reward functions"

# Launch the worker script on all nodes simultaneously using srun
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_mind_verifiable.sh
