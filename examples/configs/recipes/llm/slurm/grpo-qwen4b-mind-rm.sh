...
#SBATCH --job-name=nemo-rl-grpo-mind-qwen4b-rm
#SBATCH --nodes=5
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-grpo-mind-qwen4b-rm-%j.out
#SBATCH --error=slurm_logs/nemo-rl-grpo-mind-qwen4b-rm-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_qwen4b_rm.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/grpo_qwen4b_mind_rm_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    uv run python examples/run_grpo_rm.py \
        --config examples/configs/recipes/llm/grpo-qwen4b-mind-rm.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_qwen4b_rm.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl/examples/configs/recipes/llm/slurm/slurm_multinode_worker_grpo_qwen4b_rm.sh
