...
#SBATCH --job-name=nemo-rl-sft-mind-qwen30ba3b-merged-tool_calling-expert
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-sft-mind-qwen30ba3b-merged-tool_calling-expert-%j.out
#SBATCH --error=slurm_logs/nemo-rl-sft-mind-qwen30ba3b-merged-tool_calling-expert-%j.err
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    uv run python examples/run_sft.py \
        --config examples/configs/recipes/llm/sft-mind-megatron-qwen30ba3b-merged-tool_calling-expert.yaml \
        2>&1 | tee -a "$LOG_FILE"
